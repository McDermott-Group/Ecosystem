        ...
        text = json.dumps(
            {
                "temps": {
                    "timeStamps": [
                        deltaT(state["datetime"] - datetime.datetime(1970, 1, 1))
                    ],
                    "t60K": [state["T_60K"]["K"]],
                    "t03K": [state["T_3K"]["K"]],
                    "tGGG": [state["T_GGG"]["K"]],
                ...
                "instruments": {
                    name: {"server": status[0], "connected": status[1]}
    ...
    def onMessage(self, payload, isBinary):
        ...
        if message["command"] == "Open Heat Switch":
            ...
            try:
                ...
                self.adrServer.logMessage(
                    "Could not convert input to temperature.", alert=True
                )
        elif message["command"] == "Stop Regulating":
            self.adrServer._cancelRegulate()
        elif message["command"] == "Set Compressor State":
            if message["on"]:
                self.adrServer.startCompressor(None)
            else:
                self.adrServer.stopCompressor(None)
            ...
            if message["minutes"] is not None:
                ...
                tempData = self.adrServer.tempDataChest.getData(
                    max(0, n - m * 60), None
                )
                ...
                text = text.replace(
                    "NaN", "null"
                )  # apparently JSON does not support nan
                self.sendMessage(text)
...
class ADRServer(DeviceServer):
    ...
    def initServer(self):
        ...
        serv_conn_func = lambda c, obj: self.serversChanged(
            *obj[1]
        )  # obj used to be (s, payload), P3 fix
        serv_disconn_func = lambda c, obj: self.serversChanged(
            *obj[1]
        )  # obj used to be (s, payload), P3 fix
        ...
        try:  # in case the device manager is not running
            ...
            yield devManager.addListener(
                listener=dev_con_changed, source=None, ID=self.ID
            )
    ...
    def initLogFiles(self):
        ...
        iso = startDatetime.isoformat().split("+")[
            0
        ]  # strip timezone (or dateStamp will fail)
        ...
        try:
            ...
            self.tempDataChest.addParameter(
                "Plot Title",
                startDatetime.strftime(
                    "ADR temperature history " "for run starting on %y/%m/%d %H:%M"
                ),
            )
        self.logMessages = []
    ...
    def initializeInstruments(self):
        ...
        if (
            hasattr(instruments["Power Supply"], "connected")
    ...
    def connectDevice(self, instrName):
        ...
        try:
            ...
            instr.connected = False
            self.logMessage(
                "Could not connect to device for " + instrName + ": " + str(e) + ".",
                alert=True,
            )
    ...
    def _refreshInstruments(self):
        ...
        for serv in [n for s, n in serverList]:
            if (
                "gpib_bus" in serv or "GPIB Bus" in serv
            ):  # or 'sim900_srs_mainframe' in serv:
    ...
    def logMessage(self, message, alert=False):
        ...
        try:
            fname = os.path.join(
                os.environ["DATA_ROOT"], self.tempDataChest.pwd()
            ) + self.ADRSettings["Start Compressor Datetime"].strftime(
                "\\log_%y%m%d_%H%M.txt"
            )
    ...
    def updateState(self):
        ...
        while self.alive:
            ...
            if (
                hasattr(instruments["Compressor"], "connected")
            ...
            try:
                self.state["magnetV"] = yield instruments[
            ...
            try:
                newTemps = [
                    numpy.float16(self.state[t]["K"])
                    for t in ["T_60K", "T_3K", "T_GGG", "T_FAA"]
                ]
                timestamp = deltaT(
                    self.state["datetime"] - datetime.datetime(1970, 1, 1)
                )
                self.tempDataChest.addData([[timestamp] + newTemps])
            except Exception as e:
                self.logMessage(
                    "Temperature recording failed: %s.\n%s"
                    % (str(e), str([[timestamp] + newTemps]))
                )
    ...
    def _cancelMagUp(self):
        ...
        self.logMessage(
            "Magging up stopped at a current of " + str(self.state["PSCurrent"]) + "."
        )
    ...
    def _magUp(self):
        ...
        if self.state["regulating"]:
            self.logMessage(
                "Currently in PID control loop regulation."
        ...
        if False in deviceStatus:
            message = (
                "Cannot mag up: At least one of the essential "
        ...
        self.logMessage(
            "Beginning to mag up to " + str(settings["current_limit"]) + " A."
        )
        while self.state["maggingUp"]:
            ...
            if self.state["PSCurrent"]["A"] < settings["current_limit"]:
                if (
                    self.state["magnetV"]["V"] < settings["magnet_voltage_limit"]
                    and abs(dI["A"] / dt) < settings["dIdt_magup_limit"]
                    and self.state["T_FAA"]["K"] < settings["magnet_max_temp"]
                ):
                    newVoltage = (
                        self.state["PSVoltage"] + settings["magup_dV"] * units.V
                    )
                ...
                self.logMessage(
                    "Finished magging up. %s reached." % str(self.state["PSCurrent"])
                )
                self.state["maggingUp"] = False
                self.factory.sendMessageToAll({"isMaggingUp": self.state["maggingUp"]})
    ...
    def _cancelRegulate(self):
        ...
        self.logMessage(
            "PID Control stopped at a current of " + str(self.state["PSCurrent"]) + "."
        )
        self.factory.sendMessageToAll({"isRegulating": self.state["regulating"]})
    ...
    def _regulate(self, temp):
        ...
        if False in deviceStatus:
            message = (
                "Cannot regulate: At least one of the essential "
                "devices is not connected. Connections: %s"
                % str(
                    [
                        deviceNames[i] + ":" + str(deviceStatus[i])
        ...
        self.logMessage(
            "Starting regulation to "
            + str(self.state["regulationTemp"])
            + " K from "
        ...
        while self.state["regulating"]:
            ...
            if numpy.isnan(self.state["T_FAA"]["K"]):
                self.logMessage(
                    "FAA temperature is not valid. " "Regulation cannot continue."
                )
            ...
            self.state["PID_cumulativeError"] += (
                T_target["K"] - self.state["T_FAA"]["K"]
            )
            self.state["PID_cumulativeError"] = min(
                self.state["PID_cumulativeError"], self.ADRSettings["PID_MaxI"], key=abs
            )  # so we dont just build this up during the mag down.
            dV = (
                self.ADRSettings["PID_KP"] * (T_target["K"] - self.state["T_FAA"]["K"])
                + self.ADRSettings["PID_KI"] * self.state["PID_cumulativeError"]
                + self.ADRSettings["PID_KD"]
                * (self.lastState["T_FAA"]["K"] - self.state["T_FAA"]["K"])
                / dT
            ) * units.V
            # hard current limit
            ...
            # hard voltage limit
            if (
                self.state["PSVoltage"] + dV
                > self.ADRSettings["voltage_limit"] * units.V
            ):
                dV = (
                    self.ADRSettings["voltage_limit"] * units.V
                    - self.state["PSVoltage"]
                )
            # steady state limit
            if dV["V"] < 0:
                dV = max(
            ...
            if runCycleAgain:
                yield util.wakeupCall(
                    max(0, self.ADRSettings["step_length"] - cycleTime)
                )
            else:
    ...
    def closeHeatSwitch(self, c):
        ...
        try:
            ...
            self.logMessage("Closing Heat Switch failed.21", alert=True)
            self.logMessage(str(e.args[0]), alert=True)
    ...
    def openHeatSwitch(self, c):
        ...
        try:
            ...
            self.logMessage("Opening Heat Switch failed.12", alert=True)
            self.logMessage(str(e.args[0]), alert=True)
    ...
    def stopCompressor(self, c):
        ...
        if self.state["PSCurrent"]["A"] < 0.5:
            ...
            self.logMessage(
                "Compressor should not be stopped with current going through the magnet.",
                alert=True,
            )
