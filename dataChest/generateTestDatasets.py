...
if False:
    d.createDataset(
        "1D_ArbType1_MyFavoriteTimeSeries",
...
if True:
    ...
    d.createDataset(
        "1D_ArbType2_DampedOscillations",
...
if True:
    ...
    net = []
    for i in np.arange(5, 6, 0.01):
        for j in np.arange(-0.5, 0.5, 0.05):
            net.append(
                [float(j), float(i), gaussian(i, 5.5 + 0.2 * np.sin(10 * j), 0.05)]
            )
    d.addData(net)  # add 100 rows of data at once d.getData() #single row
...
if False:
    ...
    d.createDataset(
        "2D_ArbType2_QubitFluxSweep2",
        [
            ("Flux Bias", [len(net)], "float64", "V"),
            ("Frequency", [len(net)], "float64", "GHz"),
        ],
        [("S21", [1], "float64", "dB")],
    )
    d.addParameter("X Label", "Flux Bias")
...
# 2D Scan
if True:
    freqList = np.arange(5, 6, 0.01)
    d.createDataset(
        "2D_Scan_QubitFluxSweepScan",
    ...
    for j in np.arange(-0.5, 0.5, 0.05):
        net.append(
            [
                ...
                gaussian(freqList, 5.5 + 0.2 * np.sin(10 * j), 0.05),
            ]
        )
    d.addData(net)  # add 100 rows of data at once d.getData() #single row
