...
ERROR_GRAMMAR = {"0": "th", "1": "st", "2": "nd", "3": "rd", "4": "th"}
...
class dataChest(dateStamp):
    def __init__(
        self, path, setWorkingDirectoryToRoot=False
    ):  # add for ability to set root path
        ...
        if self._formatFilename(directoryToMake, " /+-.") == directoryToMake:
            if directoryToMake.lower() not in dirContents:
                if not os.path.isdir(self.cwdPath + "/" + directoryToMake):
                    os.mkdir(
                        self.cwdPath + "/" + directoryToMake
                    )  # Try except this even though safe guarded
            else:
                raise OSError(
    ...
    def ls(self):
        ...
        for item in cwdContents:
            if not item.startswith("."):  # ignore hidden sys files
                ...
                if (
                    ".hdf5" in pathToItem
                ):  # os.path.isfile(os.path.join(self.cwdPath,item)) too slow
                    ...
                    for rowIndex in range(0, numRows):
                        for colNum in range(0, len(data[rowIndex])):
                            if colNum < numIndeps:
                                ...
                                varGrp = "dependents"
                                varName = self.varDict[varGrp]["names"][
                                    colNum - numIndeps
                                ]
                                varData = np.asarray(data[rowIndex][colNum])
                                varShapes = self.varDict[varGrp]["shapes"]
                                flatLen = self._flatShape(
                                    varShapes[colNum - numIndeps]
                                )[0]
                                self._addToDataset(
                                    self.file[varGrp][varName],
                                    varData,
    ...
    def getData(self, startIndex=np.nan, stopIndex=np.nan, variablesList=None):
        ...
        if self.currentHDF5Filename is not None:
            ...
            for varTypes in list(self.varDict.keys()):
                ...
                for variables in desiredVarList:
                    ...
                    if len(originalShape) > 1 or originalShape != [1]:
                        for ii in range(
                            0, numRows
                        ):  # need to add similar .value here too...
                            chunk = np.asarray(
                                dataset[ii * chunkSize : (ii + 1) * chunkSize]
                            )
            ...
            if variablesList is not None:
                ...
                allVars = (
                    self.varDict["independents"]["names"]
            ...
            if self.getDataCategory() == "Arbitrary Type 1":
                ...
                for ii in range(startIndex, stopIndex):  # making slicing efficient
                    ...
                    for jj in range(0, len(allVars)):
                        row.append(
                            dataDict[allVars[jj]][ii]
                            if isinstance(allVars[jj], str)
                            else allVars[jj].decode("utf-8")
                        )
                    data.append(row)
                return data
        else:
                ...
                for item in varGroupAttributes:
                    ...
                    if item == "shapes":
                        ...
                        if type(tempList[0]) == str:
                            ...
                            self.varDict[varType][str(item)] = varGrp.attrs[
                                item
                            ].tolist()
                    else:
        ...
        if self.readOnlyFlag == True:
            ...
            if paramName in list(self.file["parameters"].keys()):
                if type(paramUnits) == str:
                    ...
                    raise IOError(
                        "Parameter units must be of type string."
                        + " "
                        + str(type(paramUnits))
                        + " are not allowed."
                    )
            else:
                raise IOError("Parameter " + str(paramName) + " was not found in file.")
        else:
            raise Warning(
    ...
    def getParameter(self, paramName, bypassIOError=False):
        if self.currentHDF5Filename is not None:
            if paramName in list(self.file["parameters"].attrs.keys()):
                paramValue = self.file["parameters"].attrs[
                    paramName
                ]  # add in type preservation here
                return paramValue
            elif paramName in list(self.file["parameters"].keys()):
                paramGrp = self.file["parameters"][paramName]
    ...
    def _initDataset(self, varDict, filename):
        ...
        for varTypes in list(varDict.keys()):
            for varAttrs in list(varDict[varTypes].keys()):
                ...
                if varAttrs == "shapes":
                    convertedShapesList = self._convertElementsToStr(
                        varDict[varTypes][varAttrs]
                    )
                    varGrp.attrs[varAttrs] = str(convertedShapesList)
                else:
                    if isinstance(varDict[varTypes][varAttrs], str):
                        varGrp.attrs[varAttrs] = str(
                            varDict[varTypes][varAttrs]
                        )  # python 2to3:
            ...
            for varTypes in list(varDict.keys()):
                ...
                for ii in range(0, len(varList)):
                    if varName == varList[ii]:
                        ...
                        convertedShapesList = self._convertElementsToStr(
                            varDict[varTypes]["units"]
                        )
                        varGrp.attrs["units"] = convertedShapesList
                        self.file[varTypes][varName].attrs["units"] = str(
                            varUnits, "utf-8"
                        )
                        return
    ...
    def _convertElementsToLists(self, listToConvert):
        conversion1 = ast.literal_eval(
            listToConvert
        )  # python 2to3: needed to convert string to list; this does the trick
        convertedList = []
    ...
    def _generateUniqueFilename(self, datasetName, dateStamp):
        ...
        while ii < maxTries and uniquenessFlag == False:
            ...
            if ii == 0:
                uniqueName = fileDateStamp + "_" + datasetName + ".hdf5"
            else:
                uniqueName = fileDateStamp + "_" + str(ii) + "_" + datasetName + ".hdf5"
    ...
    def _varListFromGrp(self, varDict):
        ...
        varList = []
        if type(shapes) == str:
            tempShapes = ast.literal_eval(
                shapes
            )  # python 2 to 3 because we removed the tolist(s) everywhere
        else:
            tempShapes = shapes
        for ii in range(0, len(names)):
            # hack for backward compatibility with N-d datasets
            if type(tempShapes[ii]) == str:  # python 2 to 3: removed .tolist()
                tempShape = ast.literal_eval(
                    (tempShapes[ii])
                )  # python 2 to 3: removed .tolist()
    ...
    def _isVarsListValid(self, varCategory, varsList):  # nonlocal
        if isinstance(varsList, list):
            if len(varsList) == 0:
                self.exception = ValueError(
                    "A data set with no " + varCategory + " has no meaning."
                )
                return False
    ...
    def _isTupleValid(self, varType, tupleValue):  # local
        if not isinstance(tupleValue, tuple):  # is it a tuple
            ...
            self.exception = ValueError(
                "Expecting tuple elements to be of length " + str(EXPECTED_TUPLE_LEN)
            )
            return False
    ...
    def _isParameterValid(self, paramName, paramValue, paramUnits, overwrite=False):
        if isinstance(paramName, str):
            if self._formatFilename(paramName, " +-.[]") != paramName:
                ...
                self._getParamterTypeString(paramValue) == "Invalid"
            ):  # self._getParamterTypeString(paramValue):
                self.exception = ValueError(
            ...
            elif overwrite is False and paramName in list(
                self.file["parameters"].attrs.keys()
            ):
    ...
    def _isDataFormatArbType1(
        self, data, varDict
    ):  # need to check that lengths are correct**** catch data = [x,y] case
        indepShapes = varDict["independents"]["shapes"]
        depShapes = varDict["dependents"]["shapes"]
    ...
    def _isDataFormat2DScan(self, data, varDict):
        ...
        for rowIndex in range(0, numRows):
            ...
            if not (
                (
                    (col1.shape == () and col2.shape == (2,))
                    or (col1.shape == (2,) and col2.shape == ())
                )
            ):
            ...
            for colIndex in range(0, totalNumVars):
                if colIndex > 1:
                    ...
                    columnShape = column.shape
                    if len(columnShape) != 1 or column.shape[0] != shapes[colIndex][0]:
    ...
    def _isDataShape2DScan(self, indepShapes, depShapes):
        if len(indepShapes) != 2:
            ...
            (indepShapes[0] == [1] and indepShapes[1] == [2])
