...
STYLE_DEFAULTS = {"font-size": "22pt", "font-color": "black"}
...
class Grapher(QtWidgets.QWidget):  # python 2to3: QtGui.QWidget >> QtWidgets.QWidget
    def __init__(self, parent=None):
        ...
        self.pluginRoot = os.path.join(
            self.pluginRoot, "servers", "dataChest", "Plugins"
        )

        self.filters = []  #         self.filters = QtCore.QStringList()
        self.filters.append("*.hdf5")
        self.dir_filters = QtCore.QRegularExpression(
            "^((?!MATLABData|TextData|TEST).)*$"
        )  # pyQt5 to pyQt6
        ...
        self.directoryTree.header().setSectionResizeMode(
            QtWidgets.QHeaderView.ResizeMode.ResizeToContents
        )  # pyQt5 to pyQt6: setResizeMode(QtWidgets.QHeaderView.ResizeToContents) >
        ...
        self.plotTypesComboBox.textActivated[str].connect(
            self.plotTypeSelected
        )  # pyQt5 to pyQt6
        ...
        )  # pyQt5 to pyQt6: self.scrollArea.setVerticalScrollBarPolicy(QtCore.Qt.ScrollBarAsNeeded)
        self.scrollArea.setHorizontalScrollBarPolicy(
            QtCore.Qt.ScrollBarPolicy.ScrollBarAsNeeded
        )  # pyQt5 to pyQt6: self.scrollArea.setHorizontalScrollBarPolicy(QtCore.Qt.ScrollBarAsNeeded)
        ...
        )  # pyQt5 to pyQt6: self.splitterVertical = QtWidgets.QSplitter(QtCore.Qt.Vertical)
        ...
        self.graphScrollArea.setVerticalScrollBarPolicy(
            QtCore.Qt.ScrollBarPolicy.ScrollBarAsNeeded
        )  # pyQt5 to pyQt6
        self.graphScrollArea.setHorizontalScrollBarPolicy(
            QtCore.Qt.ScrollBarPolicy.ScrollBarAsNeeded
        )  # pyQt5 to pyQt6
    ...
    def eventFilter(self, QObject, QEvent):
        if (
            QObject == self.dirTreeWidget
            and QEvent.type() == QtGui.QKeyEvent.Type.KeyRelease
        ):  # pyQt5 to pyQt6
    ...
    def applyPlugins(self):
        ...
        pluginList = [
            str(self.pluginTypesList.item(i).text())
    ...
    def populatePluginList(self, *args):
        for pluginFilename in os.listdir(self.pluginRoot):
            ...
            )  # pyQt5 to pyQt6: listItem.setFlags(QtCore.Qt.ItemIsUserCheckable)
            listItem.setCheckState(
                QtCore.Qt.CheckState.Unchecked
            )  # pyQt5 to pyQt6: listItem.setCheckState(QtCore.Qt.Unchecked)
            listItem.setFlags(
                QtCore.Qt.ItemFlag.ItemIsEnabled
            )  # pyQt5 to pyQt6: listItem.setFlags(QtCore.Qt.ItemIsEnabled)
                ...
                for i in range(len(self.indepVarsList)):
                    if self.indepVarsList[i][1] == [1]:
                        ...
                        if scanType is None:
                            ...
                            data[j][i] = np.logspace(
                                np.log10(start), np.log10(stop), num=l
                            )
    ...
    def populateParameterTable(self):
        ...
        )  # .setSectionResizeMode(QtWidgets.QHeaderView.ResizeMode.ResizeToContents) # pyQt5 to pyQt6
        self.parameterTable.horizontalHeader().setSectionResizeMode(
    ...
    def updatePlotTypeOptions(self, plotType):
        ...
        alreadySelected = set([var[0] for var in self.depVarsList]).intersection(
            self.selectedDepVars
        )
        if len(alreadySelected) < 1:
            ...
            self.selectedDepVars = self.getVarsWithCommonShapeList(
                firstVarList, firstVarList[0]
            )
        ...
        for var in self.depVarsList:
            if plotType == "1D" or plotType == "Histogram":
                ...
                checkBox = QtWidgets.QCheckBox(
                    varstr, self
                )  # widget to log python 2to3: varstr hack for nump.bytes_
                checkBox.toggled.connect(
                    partial(self.varStateChanged, varstr)
                )  # python 2to3: varstr hack for nump.bytes_
    ...
    def extractIndepData(self, data):
        ...
        """
        indepData = [
            np.unique(np.array(data[i])) for i in range(len(self.indepVarsList))
        ]
        ...
        for row in data.T:  # go though each row and fill data
            ...
            indepIndicies = tuple(
                [
                    np.where(indepData[i] == indepVals[i])[0][0]
                    for i in range(len(indepVals))
                ]
            )
    ...
    def plot1D(self):
        ...
        if self.cb is not None:
            self.cb.hide()
        varNames = [
            var[0].decode("UTF-8") if isinstance(var[0], np.bytes_) else var[0]
            for var in self.depVarsList
        ]  # python 2to3: decode numpy.bytes_ strings
        varUnits = [
            var[3].decode("UTF-8") if isinstance(var[3], np.bytes_) else var[3]
            for var in self.depVarsList
        ]  # python 2to3: decode numpy.bytes_ strings
        indicies = [
            varNames.index((var if isinstance(var, str) else var.decode("UTF-8"))) + 1
            for var in self.selectedDepVars
        ]
        commonUnit = varUnits[indicies[0] - 1]
        yVals = self.selectedData[indicies, :]
        pOptions = {
            "X Scale": "Linear",
            "X Label": self.indepVarsList[0][0].decode("UTF-8")
            if isinstance(self.indepVarsList[0][0], np.bytes_)
            else self.indepVarsList[0][0],  # python 2to3: decode numpy.bytes_ strings
            "X Units": self.indepVarsList[0][3].decode("UTF-8")
            if isinstance(self.indepVarsList[0][3], np.bytes_)
            else self.indepVarsList[0][3],  # python 2to3: decode numpy.bytes_ strings
            "Y Scale": "Linear",
...
<<<<<<< Updated upstream
        ...
        self.p.setLabel(
            "left", pOptions["Y Label"], units=pOptions["Y Units"], **STYLE_DEFAULTS
        )
        self.p.getAxis("bottom").setStyle(
...
>>>>>>> Stashed changes
        ...
        self.proxy = pg.SignalProxy(
            self.p.scene().sigMouseMoved, rateLimit=60, slot=self.mouseMoved
        )
    ...
    def plot2D(self):
        ...
        index = varNames.index(
            self.selectedDepVars[0].decode("UTF-8")
            if isinstance(self.selectedDepVars[0], np.bytes_)
            else self.selectedDepVars[0]
        )  # python 2to3: decode numpy.bytes_ strings
        self.graphicsLayout.clear()
        axis = {}
        for i in range(len(self.indepVarsList)):
            if self.indepVarsList[i][2] == "utc_datetime":
                axis = {
                    ["bottom", "left"][i]: TimeAxisItem(
                        orientation=["bottom", "left"][i]
                    )
                }
        ...
        self.p.setLabel(
            "bottom",
            self.indepVarsList[0][0].decode("UTF-8")
            if isinstance(self.indepVarsList[0][0], np.bytes_)
            else self.indepVarsList[0][0],
            units=self.indepVarsList[0][3].decode("UTF-8")
            if isinstance(self.indepVarsList[0][3], np.bytes_)
            else self.indepVarsList[0][3],
            **STYLE_DEFAULTS
        )  # python 2to3: decode numpy.bytes_ strings
        self.p.setLabel(
            "left",
            self.indepVarsList[1][0].decode("UTF-8")
            if isinstance(self.indepVarsList[1][0], np.bytes_)
            else self.indepVarsList[1][0],
            self.indepVarsList[1][3].decode("UTF-8")
            if isinstance(self.indepVarsList[1][3], np.bytes_)
            else self.indepVarsList[1][3],
            **STYLE_DEFAULTS
        )  # python 2to3: decode numpy.bytes_ strings
        self.p.getAxis("bottom").setStyle(
            tickTextOffset=22, tickFont=QtGui.QFont().setPointSize(22)
        ...
        # bipolar colormap
        pos = np.array([0.0, 0.125, 0.375, 0.667, 0.933, 1.0])
        color = np.array(
            [
                [0, 0, 143, 255],
                [0, 0, 255, 255],
    ...
    def getListOfAvailabePlotTypes(self, selectedVarsList=None):
        ...
        if len(self.indepVarsList) == 1:
            if (
                len(selectedVarsList) >= 2
            ):  # 1D Traj Plot Types require 2 selected dependent vars
            ...
            if len(selectedVarsList) >= 2:
                return [
                    ...
                    "2D Parametric",
                    "Projection",
                ]  # 3D Parametric for > 3 indeps
            elif len(selectedVarsList) == 1:
                return ["2D Scan", "3D Contour", "Projection"]
        else:
            return [
                "There are no available plot types for "
                + str(len(self.indepVarsList))
                + " dimensional data."
            ]
    ...
    def mouseMoved(self, evt):
        ...
        if len(self.indepVarsList) == 1:
            self.coBox.setText(
                str(round(self.xMouseVal, 3)) + ", " + str(round(self.yMouseVal, 3))
            )
        elif len(self.indepVarsList) == 2:
...
class ColorBar(pg.GraphicsObject):
    def __init__(
        ...
        self.br = p.boundingRect(
            0, 0, 0, 0, pg.QtCore.Qt.AlignmentFlag.AlignRight, label
        )
        self.zone = (
            int(mintx - 50),
            int(-15.0),
            int(75),
            int(h + self.br.height() + 30.0),
        )
        ...
        for stop, color in zip(stops, colors):
            grad.setColorAt(
                (1.0 - stop), pg.QtGui.QColor(*[int(255 * c) for c in color])
            )
        ...
        for tick, tick_label in zip(ticks, tick_labels):
            ...
            br = p.boundingRect(
                0, 0, 0, 0, pg.QtCore.Qt.AlignmentFlag.AlignRight, tick_label
            )
            if br.x() < mintx:
                mintx = br.x()
        ...
        # draw label
        p.drawText(
            int(-self.br.width() / 2.0 - 10), int(h + self.br.height() + 5.0), label
        )
        # done
        p.end()
