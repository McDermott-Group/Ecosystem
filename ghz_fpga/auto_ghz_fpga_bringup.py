...
def _parse_arguments():
    parser = argparse.ArgumentParser(
        description="Automatically " "bring-up the FPGA GHz boards."
    )
    parser.add_argument("--password", default=None, help="LabRAD password")
...
def bringup_boards(fpga, boards, ignoreLVDS=False):
    ...
    for board in boards:
        ...
        while k < NUM_TRIES and not all(successDict.values()):
            k += 1
            tries[board] = k
            boardType, result, successDict = bringup_board(fpga, board, ignoreLVDS)
            successes[board] = successDict

    failures = [board for board in boards if not all(successes[board].values())]
...
def auto_bringup(fpga, ignoreLVDS=False):
    ...
    successes, failures, tries = bringup_boards(
        fpga, [board[1] for board in boards], ignoreLVDS
    )
    return successes, failures, tries
...
def main(ignoreLVDS=False):
    ...
    with labrad.connect(password=password) as cxn:
        ...
        while no_success:
            ...
            successes = [
                board for board in list(successes.keys()) if board not in failures
            ]
            if successes:
                print(("The following boards have been succesfully " + "brought up:"))
