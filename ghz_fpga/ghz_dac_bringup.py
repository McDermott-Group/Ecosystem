...
def bringupReportMessage(dac, data):
    ...
    # BIST
    report += "\r\n".join(
        ["DAC %s BIST:" % dac, " Success: %s" % str(data["bistSuccess"]), "", ""]
    )
    return report


def bringupBoard(
    fpga, board, printOutput=True, optimizeSD=False, sdVal=None, fullOutput=False
):
    ...
    if board in fpga.list_adcs():
        ...
        return (
            "DAC",
            results,
            {"bist": all(bistOkay), "fifo": all(fifoOkay), "lvds": all(lvdsOkay)},
        )
...
def bringupBoards(fpga, boards, noisy=False):
    ...
    for board in boards:
        ...
        while tries < NUM_TRIES and not allOk:
            ...
            boardType, result, successDict = bringupBoard(
                fpga, board, fullOutput=False, printOutput=True
            )
    ...
    for board in boards:
        ...
        if triesDict[board] > 1:
            print(
                "WARNING: Board %s took %d tries to succeed" % (board, triesDict[board])
            )
    ...
    if boardType == "DAC":
        ...
        while True:
            ...
            if k in "1234567":
                ...
                if k == "2":
                    fpga.dac_debug_output(
                        0xF7FFDFFF, 0x07FFDFFF, 0x07FFDFFF, 0x07FFDFFF
                    )
                if k == "3":
                    fpga.dac_debug_output(
                        0xF8002000, 0x08002000, 0x08002000, 0x08002000
                    )
                if k == "4":
                    fpga.dac_debug_output(
                        0xFFFFFFFF, 0x0FFFFFFF, 0x0FFFFFFF, 0x0FFFFFFF
                    )
                ...
                def makeSines(freqs, T):
                    ...
                    sram = [
                        (int(round(0x1FFF * y / len(freqs))) & 0x3FFF) * 0x4001
                        for y in wave
                    ]
                ...
                if k == "5":
                    fpga.dac_run_sram(makeSines([0.100], 40), True)
                if k == "6":
                    fpga.dac_run_sram(makeSines([0.200], 40), True)
                if k == "7":
                    fpga.dac_run_sram(makeSines([0.100, 0.175], 40), True)
            ...
            if k == "I":
                bringupBoard(fpga, board, fullOutput=True)
            if k == "Q":
                break
