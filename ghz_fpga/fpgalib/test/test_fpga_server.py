...
class TestFPGAServer(object):
    ...
    def test_basic_run(self):
        ...
        matching_jt = jump_table.JumpTable(
            ...
            jumps=[
                jump_table.JumpEntry(
                    256 // 4 + _JUMP_TABLE_END_ADDR_OFFSET, 0, jump_table.END()
                )
            ],
            counters=[0, 0, 0, 0],
        )
        ...
        assert np.array_equal(
            load_writes[1][2:], np.fromstring(sram_data.tostring(), dtype="u1")
        )
    ...
    def test_multiple_boards(self):
        ...
        matching_jt = jump_table.JumpTable(
            ...
            jumps=[
                ...
                jump_table.JumpEntry(
                    512 // 4 + _JUMP_TABLE_END_ADDR_OFFSET, 0, jump_table.END()
                ),
            ],
            counters=[0, 0, 0, 0],
        )
    ...
    def _fake_run_sequence(self):
        ...
        self.runners = [
            dev.buildRunner(self.global_reps, c.get(dev, {})) for dev in devs
        ]
        ...
        for runner in self.runners:
            ...
            self.run_packets.append(
                runner.runPacket(
                    page=0,
                    slave=int(not is_master),
                    delay=self.global_board_delay,
                    sync=249,
                )
            )
            is_master = False
