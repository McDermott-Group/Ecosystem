...
class ADC(fpga.FPGA):
    ...
    def connect(self, name, group, de, port, board, build):
        ...
        print(("connecting to ADC board: %s (build #%d)" % (self.macFor(board), build)))
...
class AdcRunner_Build1(AdcRunner):
    def __init__(self, dev, reps, runMode, startDelay, filter, channels):
        ...
        if self.runMode == "average":
            ...
            raise Exception(
                "Unknown run mode '%s' for board '%s'"
    ...
    def extract(self, packets):
        ...
        if self.runMode == "average":
            ...
            return self.dev.extractDemod(packets, self.dev.DEMOD_CHANNELS_PER_PACKET)
...
class ADC_Build1(ADC_Branch1):
    ...
    def buildRunner(self, reps, info):
        ...
        channels = dict((i, info[i]) for i in range(self.DEMOD_CHANNELS) if i in info)
        runner = self.RUNNER_CLASS(self, reps, runMode, startDelay, filter, channels)
        return runner
    ...
    def setup(self, filter, demods):
        ...
        setupState = "%s: filter=%s, trigAmps=%s" % (
            self.devName,
            filterFunc.tostring(),
            amps,
        )
    ...
    def runAverage(self, filterFunc, filterStretchLen, filterStretchAt, demods):
        ...
        def func():
            ...
            regs = self.regRun(
                mode=self.RUN_MODE_AVERAGE_AUTO,
                reps=1,
                filterFunc=filterFunc,
                filterStretchAt=filterStretchAt,
                filterStretchLen=filterStretchLen,
    ...
    def runDemod(self, filterFunc, filterStretchLen, filterStretchAt, demods):
        ...
        def func():
            ...
            # of 48-byte strings
            packets = [data for src, dst, eth, data in ans.read]
            returnValue(self.extractDemod(packets, self.DEMOD_CHANNELS_PER_PACKET))
...
class AdcRunner_Build7(AdcRunner_Build2):
    def __init__(self, dev, reps, runMode, startDelay, channels, info):
        ...
        if self.runMode == "average":
            ...
            self.mode = self.dev.RUN_MODE_DEMOD_DAISY
            iqPairsPerExpt = sum(
                [rcount * rchan for rcount, rdelay, rlen, rchan in info["triggerTable"]]
            )
            packetsPerExpt = int(
                np.ceil(iqPairsPerExpt / float(self.dev.DEMOD_CHANNELS_PER_PACKET))
            )
        ...
        statTime = 4e-9 * sum(
            [
                count * (delay + rlen)
                for count, delay, rlen, chan in info["triggerTable"]
            ]
        )
    ...
    def runPacket(self, page, slave, delay, sync):
        ...
        startDelay = self.startDelay + delay
        regs = self.dev.regRun(
            mode=self.mode, info=self.info, reps=self.reps, startDelay=startDelay
        )
    ...
    def extract(self, packets):
        ...
        if self.runMode == "average":
            ...
            return self.dev.extractDemod(
                packets, self.info["triggerTable"], self.info.get("mode", "iq")
            )
...
class ADC_Branch2(ADC):
    ...
    def makeTriggerTable(cls, triggerTable, p):
        ...
        if any(rlens):
            ...
            if (
                rlen0 < 50 and count0 == 1 and not any(rlens[1:])
...
class ADC_Build7(ADC_Branch2):
    ...
    def buildRunner(self, reps, info):
        ...
        startDelay = info["startDelay"]
        channels = dict((i, info[i]) for i in range(self.DEMOD_CHANNELS) if i in info)
        runner = self.RUNNER_CLASS(self, reps, runMode, startDelay, channels, info)
        return runner
    ...
    def setup(self, info):
        ...
        triggerTableState = " ".join(
            [
                "triggerTableState%d=%s" % (idx, triggerTable[idx])
                for idx in range(len(triggerTable))
            ]
        )
        mixTableState = " ".join(
            [
                "mixTable%d=%s" % (idx, demod["mixerTable"])
                for idx, demod in enumerate(demods)
            ]
        )
    ...
    def registerReadback(self):
        ...
        def func():
            # build registry packet
            regs = self.regRun(mode=self.RUN_MODE_REGISTER_READBACK, info={}, reps=0)
    ...
    def runAverage(self):
        ...
        def func():
            # build registry packet
            regs = self.regRun(mode=self.RUN_MODE_AVERAGE_AUTO, info={}, reps=1)
    ...
    def runDemod(self, info):
        ...
        def func():
            # build register packet
            regs = self.regRun(mode=self.RUN_MODE_DEMOD_AUTO, info=info, reps=1)
            # create packet for the ethernet server
    ...
    def extractDemod(cls, packets, triggerTable, mode):
        ...
        pkt_per_stat = int(
            np.ceil((totalTriggers * rchan) / float(cls.DEMOD_CHANNELS_PER_PACKET))
        )
        ...
        if len(packets) % pkt_per_stat:
            raise RuntimeError(
                "wrong number of packets: %d not a multiple of pkt_per_stat: %d"
                % (len(packets), pkt_per_stat)
            )
        ...
        stat_pkt_list = [
            packets[idx * pkt_per_stat : pkt_per_stat * (idx + 1)]
        ...
        for stat_packet in stat_pkt_list:
            data = np.fromstring(
                "".join(data[:44] for data in stat_packet), dtype="<u1"
            )
            ...
            if mode == "iq":
                ...
                """
                raise RuntimeError(
                    "Operation mode %s not implemented / available" % (mode,)
                )
        ...
        return (
            ...
            pktCounters,
            readbackCounters,
        )  # Only returning the packet counters of the last stat.  FIXME if you care about these
