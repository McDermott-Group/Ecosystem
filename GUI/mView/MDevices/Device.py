                ...
                if self.warning.consent:
                    ...
                    if actual_button[3] is not None:
                        resp = getattr(self.deviceServer, actual_button[1])(
                            actual_button[3], context=self.ctx
                        )
            ...
            try:
                ...
                for i, name in enumerate(self.getParameters()):
                    ...
                    if command[1] is not None:
                        reading = getattr(self.deviceServer, command[0])(
                            command[1], context=self.ctx
                        )
                    else:
                        reading = getattr(self.deviceServer, command[0])(
                            context=self.ctx
                    ...
                    if isinstance(reading, ValueArray):
                        ...
                        units = reading.units
                        if index != None and isinstance(reading[index], Value):
                    ...
                    if isinstance(reading, Value):
                        ...
                        if preferredUnits is not None and reading.isCompatible(
                            preferredUnits
                        ):
                        ...
                        for j in range(len(reading)):
                            ...
                            if isinstance(rd, Value):
                                preferredUnits = self.getPreferredUnit(name)
                                if preferredUnits is not None and rd.isCompatible(
                                    preferredUnits
                                ):
                        ...
                        try:
                            ...
                            print(
                                (
                                    "Problem with readings, type '%s' "
                                    "cannot be displayed." % str(type(reading))
                                )
                            )
                ...
                print(
                    (
                        "[%s] Something appears to be wrong with what "
                        "the labrad server is returning." % str(self.frame.getTitle())
                    )
                )

            except:
                traceback.print_exc()
                self.frame.raiseError("Problem communicating with %s." % self.name)
