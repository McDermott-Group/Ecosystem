...
class IMPA_GUI(QtGui.QWidget):
    ...
    def initialize_sim_rack_selections(self):  # make all sim related stuff into class
        ...
        self.voltage_spinbox.setRange(-2.0, 2.0)
        self.voltage_spinbox.setValue(0.0)
        self.voltage_spinbox.setDecimals(3)
        self.voltage_spinbox.setSingleStep(0.001)
        self.voltage_spinbox.valueChanged.connect(
            partial(call_labrad_setting, self.sim928.set_voltage, V)
        )
        ...
        self.voltage_increment.activated[str].connect(
            partial(
                increment_updater,
    ...
    def initialize_rf_generator_selections(
        self,
    ):  # make all rf generator related stuff into class
        ...
        self.rf_generator_frequency_increment_title = QtGui.QLabel(
            "Frequency Increment:"
        )
        ...
        self.frequency_spinbox.setRange(2.0, 14.0)
        ...
        self.frequency_spinbox.valueChanged.connect(
            partial(call_labrad_setting, self.rf_generator.frequency, GHz)
        )
        ...
        self.power_spinbox.setRange(-80, 0.0)
        ...
        self.power_spinbox.valueChanged.connect(
            partial(call_labrad_setting, self.rf_generator.power, dBm)
        )
        self.power_spinbox.setEnabled(False)

        self.rf_generator_power_increment.activated[str].connect(
            partial(
                increment_updater, self.power_spinbox, POWER_INCREMENT_STRING_TO_FLOAT
            )
        )

        self.rf_generator_frequency_increment.activated[str].connect(
            partial(
                increment_updater,
    ...
    def select_rf_generator(self, selection):
        ...
        if (
            selection != "Please Select an RF Generator"
            and selection != self.selected_rf_generator
        ):
    ...
    def select_sim_900_mainframe(self, selection):
        selection = str(selection)
        if (
            selection != "Please Select a Mainframe"
            and selection != self.selected_sim_rack
        ):
    ...
    def select_sim_928_slot(self, selection):
        selection = str(selection)
        if (
            selection != "Please Select a Slot Number"
            and selection != self.selected_sim_slot
        ):
            if self.selected_sim_slot is None:
                self.sim_slot_selection.removeItem(0)
...
def call_labrad_setting(
    labrad_setting, units, value
):  # all three set functions are essentially the same
    value = float(value)
    labrad_setting(value * units)


def increment_updater(
    spinbox, mapping, increment
):  # all three set functions are essentially the same
