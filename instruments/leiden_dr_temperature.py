...
class LeidenDRPseudoserver(LabradServer):
    ...
    def getRegistryKeys(self):
        ...
        elif "Leiden Log Files Path" not in keys or not os.path.exists(self._path):
            self._path = (
                r"Z:\mcdermott-group\data\fridgeLogs\dr2" "\resistanceBridgeRawData"
            )
        if not os.path.exists(self._path):
            raise Exception(
                "Could not find the Leiden Log Files " "Path: '%s'" % str(self._path)
            )
    ...
    def readTemperatures(self):
        ...
        with open(os.path.join(self._path, file), "rb") as f:
            ...
            if (
                ...
                or self._arr_exch[-1] != raw_exch
                or self._arr_mix[-1] != raw_mix
                or self._arr_mix_pt1000[-1] != raw_mix_pt1000
            ):
    ...
    def filteredTemperature(self, array, lower_threshold, upper_threshold):
        if not np.any(np.isfinite(array)):
            return np.nan
        # Raw thresholding.
        array = array[np.isfinite(array)]
        mask = np.logical_and(
            np.less(array, upper_threshold), np.greater(array, lower_threshold)
        )
        raw = array[mask]
        ...
        # Fine thresholding.
        if filtered.size:
            weight = np.exp(-np.linspace(filtered.size / 5.0, 0, filtered.size))
