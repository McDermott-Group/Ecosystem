...
DLL_path = os.path.join(area51_root, "instruments\\labbricks\\vnx_fmsynth.dll")
...
fnLMS_GetDevInfo = getDllObject(
    "fnLMS_GetDevInfo", [POINTER(ACTIVEDEVICES)], restype=c_int
)
...
fnLMS_SetUseExternalPulseMod = getDllObject(
    "fnLMS_SetUseExternalPulseMod", [DEVID, c_bool]
)
fnLMS_SetFastPulsedOutput = getDllObject(
    "fnLMS_SetFastPulsedOutput", [DEVID, c_float, c_float, c_bool]
)
...
fnLMS_GetUseInternalPulseMod = getDllObject(
    "fnLMS_GetUseInternalPulseMod", restype=c_int
)
...
class LabBrick_Synthesizer:
    ...
    def initDevice(self, serial):
        ...
        self.maxFreq = float(fnLMS_GetMaxFreq(self.device_id)) * 10.0
        self.minFreq = float(fnLMS_GetMinFreq(self.device_id)) * 10.0
    ...
    def setInternalPulseMod(self, dOntime, dReptime, bOn=False):
        status = fnLMS_SetFastPulsedOutput(
            self.device_id, c_float(dOntime), c_float(dReptime), c_bool(bOn)
        )
        self.check_error(status)
    ...
    def check_error(self, status=0):
        ...
        dError = {
            BAD_PARAMETER: "Out of range input - frequency outside min/max etc.",
