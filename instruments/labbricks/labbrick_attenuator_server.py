...
class LBAttenuatorServer(LabradServer):
    ...
    def initServer(self):
        ...
        self.DLL_path = os.path.join(
            area51_root, "instruments\\labbricks\\VNX_atten64.dll"
        )
        self.DLL_path_old = os.path.join(
            area51_root, "instruments\\labbricks\\VNX_atten64.dll"
        )  # this will FAIL when attempting to get device ID; function no longer exists in DLL.
        self.autoRefresh = True
        try:
    ...
    def refreshAttenuators(self):
        ...
        if n == self._num_devs:
            ...
            for idx in range(0, n):
                ...
                print(
                    (
                        "Found a %s Lab Brick Attenuator, serial "
    ...
    def getDeviceDID(self, c):
        if "SN" not in c:
            raise DeviceNotSelectedError(
                "No Lab Brick Attenuator " "serial number is selected"
            )
        if c["SN"] not in list(self._SN2DID.keys()):
            raise Exception(
                "Cannot find Lab Brick Attenuator with " "serial number %s" % c["SN"]
            )
    ...
    def attenuation(self, c, attn=None):
        ...
        if attn is not None:
            attn_value = ctypes.c_int(int(4.0 * attn["dB"]))
            yield self.VNXdll.fnLDA_SetAttenuation(DID, attn_value)
        attn = 0.25 * (yield self.VNXdll.fnLDA_GetAttenuation(DID)) * dB
