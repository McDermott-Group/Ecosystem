...
class Lakeshore218Wrapper(GPIBDeviceWrapper):
    ...
    def getTemp(self, channel=0, unit="K"):
        ...
        if unit == "K":
            temp = [float(t) * units.K for t in resp.split(",")]
        elif unit == "C":
            temp = [float(t) * units.C for t in resp.split(",")]
        elif unit == "S":
            temp = [float(t) * units.V for t in resp.split(",")]
        else:
            temp = resp
        returnValue(temp)
...
class Lakeshore218Server(GPIBManagedServer):
    ...
    @setting(11, "Get Temperature", channel="v", unit="s", returns="?")
    def getTemp(self, c, channel=0, unit="K"):
        """Returns the temperatures for a given channel (or a list of all channels if channel=0)
        in the specified units (K,C,Linear(L),System(S))."""
