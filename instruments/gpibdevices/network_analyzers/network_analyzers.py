...
class AgilentN5230ADeviceWrapper(ReadRawGPIBDeviceWrapper):
    ...
    def sweep_type(self, stype="LIN"):
        ...
        if stype is None:
            ...
            if stype.upper() != "CW" and stype.upper() != "LIN":
                raise ValueError(
                    "Unknown sweep type: %s. " + 'Please use "LIN" or "CW".' % stype
                )
            else:
    ...
    def measurement_setup(
        self, s_params=["S11"], formats=["MP"], trigger="IMM", electrical_delay_ns=None
    ):
        """
        ...
        for Sp in s_params:
            if Sp not in self.available_traces:
                raise ValueError("Illegal S-parameter: %s." % str(Sp))
        ...
        for k, Sp in enumerate(s_params):
            if formats[k] == "RI":
                ...
                if electrical_delay_ns is not None:
                    yield self.write(
                        "CALC:CORR:EDEL:TIME " + str(int(electrical_delay_ns)) + "NS"
                    )
    ...
    def get_data(self):
        ...
        data = ()
        pair = ()
        unit_multipliers = {"R": 1, "I": 1, "M": units.dB, "P": units.deg}
...
class KeysightE5063ADeviceWrapper(AgilentN5230ADeviceWrapper):
    ...
    @inlineCallbacks
    def measurement_setup(self, s_params=["S11"], formats=["MP"], trigger="IMM"):
        """
        Setup the measurement.
        ...
        if num_params == 1:
            ...
            raise ValueError(
                "Only one or two S-parameters are allowed "
    ...
    def get_data(self):
        ...
        # End any previous measurements
        yield self.write("INIT1:CONT 0")
        yield self.write("ABOR")

        # Start measurement (edited by GS and DH 5/22/23)
        yield self.write("TRIG:SOUR BUS")
        yield self.write("INIT1:CONT 1")
        yield self.write("TRIG:SING")
        ...
        while measurement_finished == False:
            yield sleep(0.05)  # polling rate = 20Hz
            opc_bit = yield self.query(
                "*OPC?", timeout=24 * units.h
            )  # poll ESR for measurement completion
            # print(opc_bit)
            opc_bit = int(opc_bit) & 0x1
            if opc_bit == 1:
                measurement_finished = True
        ...
        data = ()
        pair = ()
        unit_multipliers = {"REAL": 1, "IMAG": 1, "MLOG": units.dB, "PHAS": units.deg}
...
class Agilent8720ETDeviceWrapper(ReadRawGPIBDeviceWrapper):
    ...
    def center_frequency(self, cfreq=None):
        ...
        if cfreq is None:
            ...
            yield self.write("CENT%i" % cfreq["Hz"])
        returnValue(cfreq)
    ...
    def frequency_span(self, span=None):
        ...
        if span is None:
            ...
            yield self.write("SPAN%i" % span["Hz"])
        returnValue(span)
    ...
    def start_frequency(self, start=None):
        ...
        if start is None:
            ...
            yield self.write("STAR%i" % start["Hz"])
        returnValue(start)
    ...
    def stop_frequency(self, stop=None):
        ...
        if stop is None:
            ...
            yield self.write("STOP%i" % stop["Hz"])
        returnValue(stop)
    ...
    def source_power(self, power=None):
        ...
        if power is None:
            ...
            if power["dBm"] < -100:
                power = -100 * units.dBm
                print("Minimum power level for %s is %s." % (self.model, power))
            if power["dBm"] > 10:
                power = 10 * units.dBm
                print("Maximum power level for %s is %s." % (self.model, power))
    ...
    def measurement_setup(self, s_params=["S11"], formats=["MP"], trigger="IMM"):
        ...
        if trigger.upper() != "IMM":
            raise ValueError(
                'Only immediate ("IMM") triggering is '
    ...
    def get_data(self):
        ...
        avgOn = yield self.average_mode()

        if avgOn:
            numAvg = yield self.average_points()
            yield self.write("AVERREST")
        ...
        raw = np.frombuffer(dataBuffer, dtype=np.float32)
        print(raw.shape[-1])
        print(raw.shape)
        data = np.empty((int(raw.shape[-1]) - 1) // 2)
        real = raw[1:-1:2]

        # get phase (GS 7/7/23) if form is log mag
        # format/return values are listed on page 1-161 here
        # https://www.keysight.com/us/en/assets/9018-40048/programming-guides/9018-40048.pdf?success=true
        if self.form == "MP":
        ...
        if self.form == "MP":
            data = ((real.astype(float) * units.dB, imag.astype(float) * units.deg),)
        elif self.form == "RI":
...
class VNAServer(GPIBManagedServer):
    ...
    deviceWrappers = {
        "AGILENT TECHNOLOGIES N5230A": AgilentN5230ADeviceWrapper,
    ...
    @setting(1500, "Measurement Setup", s_params="*s", formats="*s", trigger="s")
    def measurement_setup(self, c, s_params=["S21"], formats=["MP"], trigger="IMM"):
        """
        Setup the measurement.
        ...
        if formats is not None:
            if len(formats) is not len(s_params):
                raise IndexError("s_params and formats are not the same " "length.")
            for form in formats:
                if form not in ("RI", "MP"):
                    raise ValueError(
                        "Illegal measurment definition: " "%s." % str(form)
                    )
        else:
