...
class TektronixTDS2014CWrapper(GPIBDeviceWrapper):
    @inlineCallbacks
    def getDataSourceStr(
        self,
    ):  # returns the channels that will return data when the scope is queried for waveform data
        result = yield self.query("DATa:SOUrce?")
        returnValue(result)

    @inlineCallbacks
    def setDataSourceStr(
        self, channelNum
    ):  # sets which channels will return data when the scope is queried for waveform data
    ...
    def encASCIIstr(
        self,
    ):  # sets data encoding to ascii (note: this is the most inefficient/convenient way to return data)
    ...
    @inlineCallbacks
    def getChannelPosition(
        self, channelNum
    ):  # Sets or returns the channel vertical position (note: this is not the same as offset)
        if channelNum in [1, 2, 3, 4]:
    ...
    @inlineCallbacks
    def setDataStartIndex(
        self, startIndex
    ):  # gets the number of elements sent by oscilloscope
    ...
    def getSamplingMode(self):
        ...
        )  # RT ==> Real-Time, IT ==> Interpolated-Time, ET ==> equivalent time (this server assumes RT)
        returnValue(result)
    ...
    @inlineCallbacks
    def setTimePerDivision(
        self, timePerDiv
    ):  ## depends on number of channels that are on, if single channel 1/(20e9) min, if two 1/(10e9) if 3 or 4 1/(5e9)
    ...
    def configureEdgeTrigger(self, triggerChanNum, slope, level):
        if slope == "Rise" or slope == "Fall":
            ...
            yield self.write(
                "TRIGger:A:LEVel:CH" + str(triggerChanNum) + " " + str(level)
            )
            returnValue(1)
        else:
            returnValue(-1)
...
class TektronixTDSServer(GPIBManagedServer):
    ...
    def configureEdgeTrigger(
        self, c, triggerChanNum, slope, level
    ):  # configureEdgeTrigger(self, triggerChanNum, slope, level)
        dev = self.selectedDevice(c)
        answer = yield dev.configureEdgeTrigger(triggerChanNum, slope, level)
        returnValue(answer)

    @setting(
        112, "getChannelState", channelNum="i", allChannelsFlag="b", returns="s"
    )  # def getChannelState(self, channelNum, allChannelsFlag = None)
    ...
    def getWaveFormData(self, c):  # waveforms should only be output upon trig ready
        ...
        splitWaveformData = (yield dev.getWaveFormDataStr()).split(
            ";"
        )  # elements 0 - 17 make up first wave form, elements 18-35 make up the second waveform, ...
