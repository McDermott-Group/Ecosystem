...
class Tektronix11801CServer(GPIBManagedServer):
    ...
    def getTraceData(self, c, traceNum=None):
        ...
        if traceNum is None:  # Assume user wants all traces.
            ...
            traceData = yield dev.query("OUTPUT TRACE%s;WAV?" % str(traceNum))
        ...
        for ii in range(0, len(splitter), 2):
            ...
            xdata = [
                XINCR * (k - 1) + XZERO for k in range(1, len(traceDataStr))
            ] * xunit
            ydata = [
                YZERO + YMULT * float(traceDataStr[k])
                for k in range(1, len(traceDataStr))
            ] * yunit
            data += ((xdata, ydata),)
