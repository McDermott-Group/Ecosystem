...
class HP3478AServer(GPIBManagedServer):
    ...
    @setting(
        11, "Get Measurement", measurement="s", range="s", trigger="s", returns="?"
    )
    def getMeasurement(
        self, c, measurement="DC volts", range="3V,3A", trigger="internal trigger"
    ):
        ...
        # print resp
        if measurement[-5:] == "volts":
            resp = float(resp) * units.V
        elif measurement[-4:] == "ohms":
            resp = float(resp) * units.Ohm
        elif measurement[-7:] == "current":
            resp = float(resp) * units.A
        returnValue(resp)
    ...
    def HP_write(self, c, data):
        ...
        servers = [s for n, s in allServers if (("GPIB Bus" in s) or ("gpib_bus" in s))]
        if "addr" not in c:
    ...
    def getRuoxTemperature(self, c):
        ...
        R = RCal * 1000 * V["V"]
        try:
            T = pow((2.85 / math.log((R - 652) / 100)), 4) * units.K
        except ValueError:
            T = numpy.nan * units.K
        returnValue(T)
