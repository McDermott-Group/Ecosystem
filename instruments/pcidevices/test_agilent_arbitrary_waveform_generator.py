...
RO_I, RO_Q = wf.Harmonic(
    amplitude=ExptVars["R0 Drive Amplitude"],
...
DWELL = wf.DC(
    amplitude=ExptVars["Dwell Amplitude"],
    start=RO_I.end,
    duration=ExptVars["Dwell Time"],
)
DWELL2 = wf.DC(
    amplitude=ExptVars["Dwell Amplitude 2"],
    start=DWELL.end,
    duration=ExptVars["Dwell Time 2"],
)

original_waveforms_dict = wf.wfs_dict(
    wf.Waveform("RO_I", RO_I),
...
keysight_waveforms_list = [
    ...
    awg.KeysightWaveform(
        slot_number=4, channel_number=4, waveform_name="RO_I"
    ),  #'ATS_Trigger'
