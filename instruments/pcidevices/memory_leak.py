...
def initialize_awg(slot_number, channel_numbers=[1, 2, 3, 4], chassis_number=0):
    ...
    for channel_number in channel_numbers:
        waveshape_error_code = awg.channelWaveShape(
            channel_number, keySD.SD_Waveshapes.AOU_AWG
        )
...
def create_keysight_waveform_objects():
    ...
    create_waveform_error_code = keysight_waveform_object1.newFromArrayDouble(
        keySD.SD_WaveformTypes.WAVE_ANALOG, waveform1.tolist()
    )
    if create_waveform_error_code < 0:
        print(("create_waveform_error_code1=", create_waveform_error_code))
    create_waveform_error_code = keysight_waveform_object2.newFromArrayDouble(
        keySD.SD_WaveformTypes.WAVE_ANALOG, waveform2.tolist()
    )
    if create_waveform_error_code < 0:
        print(("create_waveform_error_code2=", create_waveform_error_code))
    create_waveform_error_code = keysight_waveform_object3.newFromArrayDouble(
        keySD.SD_WaveformTypes.WAVE_ANALOG, waveform3.tolist()
    )
    if create_waveform_error_code < 0:
        print(("create_waveform_error_code3=", create_waveform_error_code))
    create_waveform_error_code = keysight_waveform_object4.newFromArrayDouble(
        keySD.SD_WaveformTypes.WAVE_ANALOG, waveform4.tolist()
    )
    if create_waveform_error_code < 0:
        print(("create_waveform_error_code4=", create_waveform_error_code))
    return (
        keysight_waveform_object1,
        keysight_waveform_object2,
        keysight_waveform_object3,
        keysight_waveform_object4,
    )
...
def queue_waveforms(
    ...
    for channel_number in channel_numbers:
        queue_waveform_error_code = awg.AWGqueueWaveform(
            channel_number, channel_number, trigger_mode, start_delay, cycles, prescalar
        )
...
for ii in range(0, 10000):
    ...
    waveforms_for_awgs2 = (
        create_keysight_waveform_objects()
    )  # all the same here but could in principle be different
